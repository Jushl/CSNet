...
class snn_Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = out[..., 0]
        zero_count = torch.sum(x0 == 0).item()
        one_count = torch.sum(x0 == 1).item()
        negative_one_count = torch.sum(x0 == -1).item()
        print('0', zero_count)
        print('1', one_count)
        print('-1', negative_one_count)
        x1 = out[..., 1]
        zero_count = torch.sum(x1 == 0).item()
        one_count = torch.sum(x1 == 1).item()
        negative_one_count = torch.sum(x1 == -1).item()
        print('0', zero_count)
        print('1', one_count)
        print('-1', negative_one_count)
